...
const EMPTY = { name: "", email: "", username: "", password: "", phone: "", status: "active", language: "fr", roleId: "none", branchIds: [] as number[], posAccess: false, adminAccess: false };
...
export default function Users() {
  ...
  function openEdit(u: User) {
    setEditing(u);
    setForm({ name: u.name, email: u.email, username: u.username, password: "", phone: u.phone ?? "", status: u.status, language: u.language, roleId: u.roleId?.toString() ?? "none", branchIds: u.branchIds, posAccess: u.posAccess, adminAccess: u.adminAccess });
    setDialogOpen(true);
  }
  function toggleBranch(id: number) { setForm(f => ({ ...f, branchIds: f.branchIds.includes(id) ? f.branchIds.filter(b => b !== id) : [...f.branchIds, id] })); }
  function save() {
    const data = { ...form, roleId: form.roleId && form.roleId !== "none" ? parseInt(form.roleId) : null };
