      ...
      <DialogContent className="max-w-md">
        <DialogHeader>
          <DialogTitle className="flex items-center gap-2">
            ...
            إشعار الأوليين عبر واتساب
          </DialogTitle>
        </DialogHeader>
        <div className="space-y-3">
          <p className="text-sm text-muted-foreground">
            تم إنشاء الطلبات بنجاح. أرسل تنبيهاً لكل عامل عبر واتساب:
          </p>
          <div className="space-y-2">
            {sentWorkers.map(w => {
              const msg = encodeURIComponent(`مرحباً ${w.name}،\n\nلديك طلب تحضير جديد (${w.count} منتج${w.count > 1 ? "ات" : ""}) في انتظارك.\nتفضل راجعه في قسم "Mes préparations" على الموقع.\n\nشكراً 🙏`);
              const href = w.phone
          ...
          {sentWorkers.some(w => !w.phone) && (
            <p className="text-xs text-amber-600 bg-amber-50 rounded px-3 py-2">
              💡 بعض العمال لا يملكون رقم واتساب. أضفه من صفحة <strong>Ouvriers</strong>.
            </p>
          )}
        </div>
        <DialogFooter>
          <Button variant="outline" onClick={() => setWaDialogOpen(false)}>إغلاق</Button>
        </DialogFooter>
      </DialogContent>
    </Dialog>
    </>
